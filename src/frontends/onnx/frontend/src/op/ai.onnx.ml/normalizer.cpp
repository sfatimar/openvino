...
ov::OutputVector normalizer(const ov::frontend::onnx::Node& node) {
     ...
     // Step 2: Get Normalization
     const auto norm = node.get_attribute_value<std::string>("norm");
     ...
     if (normalization_type == "L1") {
         const auto sum_x = std::make_shared<ov::op::v1::ReduceSum>(input, axes, true);
         const auto safe_x = check_zero_divisor(sum_x);
         return {std::make_shared<ov::op::v1::Divide>(input, safe_x)};
     } else if (normalization_type == "L2") {
         const auto x_squared = std::make_shared<ov::op::v1::Multiply>(input, input);
         const auto sum_sqrdx = std::make_shared<ov::op::v1::ReduceSum>(x_squared, axes, true);
         const auto safe_x = check_zero_divisor(sum_sqrdx);
         const auto result = std::make_shared<ov::op::v1::Divide>(x_squared, safe_x);
         return {std::make_shared<ov::op::v0::Sqrt>(result)};
     } else {  // Must be max
         const auto max_x = std::make_shared<ov::op::v1::ReduceMax>(input, axes, /*keep_dims=*/true);
