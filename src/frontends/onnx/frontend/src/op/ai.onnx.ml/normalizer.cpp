...
ov::OutputVector normalizer(const ov::frontend::onnx::Node& node) {
     ...
     const auto x = std::make_shared<ov::op::v0::Abs>(input);
     ...
     if (normalization_type == "L1") {
         ...
         return {std::make_shared<ov::op::v1::Divide>(x, safe_x)};
         ...
         const auto result = std::make_shared<ov::op::v1::Divide>(x, safe_x);
         return {result};
     } else {  // Must be max
